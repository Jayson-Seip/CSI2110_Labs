...
public class AlgAnalysis {
	...
	public static void arraySortRuntime(int count, int maxSize){
		...
		for(int i=1; i<=count; i ++){
			int n = (maxSize/count)*i;
			for(int j=0;j<10;j++){
				int[] arr = randomizeArray(genArray((maxSize/count)*i));
			...
			double nSquared =  (double)minTime/(double)(n*n);
			double nLogn =  (double)minTime/(n*(Math.log(n)/Math.log(2)));
			System.out.println("n: "+(maxSize/count)*i+", T(n): "+(double)minTime+" NanoSeconds,T(n)/n*n: "+nSquared+", T(n)/nlog(n): "+nLogn);
